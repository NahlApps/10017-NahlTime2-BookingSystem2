...
/* ========================================================================== */
/*  2) REVIEW FEATURE: CONSTANTS & API                                        */
/* ========================================================================== */
...
/* ========================================================================== */
/*  5) UI HELPERS (TOAST / LAYOUT / PROGRESS)                                 */
/* ========================================================================== */
...
function syncProgress(i){
  ...
  document.getElementById('miniStepTitle').textContent =
...
/* Loading indicators for times/services */
function setLoadingTimes(on){
  document.getElementById('timeLoading').classList.toggle('show', !!on);
}
function setLoadingServices(on){
  document.getElementById('servicesLoading').classList.toggle('show', !!on);
}

/* ========================================================================== */
/* 11) SPECIAL PLATE DETECTION                                                */
/* ========================================================================== */
...
function updatePlateHint(){
  ...
  $('#plateNumber').val(v);
  const txt = v
    ? (isSpecialPlate(v) ? 'رقم مميز 👌 — ممتاز للتوثيق السريع.' :
...
function startWelcomeDeck(){
  ...
  const setActive=(i,ex=-1)=>{
    slides.forEach((s,idx)=>{
      ...
      if(idx===i){
        s.classList.add('is-active'); s.style.zIndex=2;
      } else if(idx===ex){
        s.classList.add('is-exiting'); s.style.zIndex=1;
      } else{
        s.style.zIndex=0;
      }
    });
...
function normalizeAvailability(r){
  ...
  if(!Number.isFinite(rem) && Number.isFinite(c) && Number.isFinite(b)){
    rem=c-b;
  }
  const open =
    f===true ||
...
function fetchTimesForSelectedDate(){
  ...
  if(!loc||!srv){
    ...
    window.selectedTime=null; selectedTime=null; updateNextAvailability();
    return;
  }
...
/* ========================================================================== */
/* 16) LAYOUT: RESIZE OBSERVER FOR HEADER/FOOTER                              */
/* ========================================================================== */
    ...
    entries.forEach(entry=>{
      if(entry.target.id==='siteHeader'){
        document.documentElement
          .style.setProperty('--header-h', entry.contentRect.height+'px');
      }
      if(entry.target.id==='siteFooter'){
        document.documentElement
          .style.setProperty('--footer-h', entry.contentRect.height+'px');
      }
    });
  });
...
async function validateCouponAndApply(){
  ...
  const customer = itiPhone && itiPhone.getNumber
    ? itiPhone.getNumber().replace(/^\+/, '')
    : '';
 ...
 * تُستخدم من:
 *  - click / tap على الخريطة
 *  - double-tap / dblclick
 *  - "إظهار موقعي"
 *  - نتائج البحث في حقل العنوان
 */
function setPinPosition(rawLatLng, opts = {}) {
  ...
  // 🛡️ التحقق من أن النقطة داخل الـ Polygon (إن وجد) – مع حماية من أي خطأ
  if (!skipBoundsCheck && areaPolygon) {
    try {
      if (
        google.maps &&
        ...
        if (!inside) {
          // نرجع لآخر نقطة صحيحة إن كانت موجودة
          if (lastValidLatLng) {
            const last =
  ...
  // 👆 click-to-drop: نحذف أي ماركر قديم ونضع واحد جديد draggable
  if (marker) {
    marker.setMap(null);
  }
  marker = new google.maps.Marker({
...
/**
 * تحميل حدود/مركز المنطقة من الـ backend
 */
async function loadAreaBounds(areaId) {
  ...
  try {
    ...
    // مركز المنطقة + وضع الماركر في المنتصف
    if (center) {
      map.setCenter(center);
      setPinPosition(center, { pan: false, skipBoundsCheck: true });
    }

    // تطبيق قيود على الخريطة لو الحدود كاملة
    if (
...
$(function(){
  ...
  $('#area').on('change', function(){
    ...
    callContent2(`/services?location=${encodeURIComponent(this.value)}`, res=>{
      ...
      const cats = categoriesCache.map(c=>({
        id:c.TS_category_id,
        text:c.TS_category_arabic_name
      }));
      $('#serviceCat').empty().select2({
        data:cats,
        width:'100%',
